...
Статус и версия разделены: используются только APPROVED, DRAFT, OPEN, NOT_STARTED. APPROVED v0.1 не означает, что пробелы исходников решены. Подразделы наследуют только уже заданный scope одобрения; новые согласованные решения зафиксированы отдельно в CANON UPDATE 001–010; APPROVED не распространяется на неописанную детализацию.
...
| Combat | DRAFT | COMBAT-001–COMBAT-030, только согласованные границы | [Core](combat/COMBAT_CORE.md), [Stats](combat/COMBAT_STATS.md), [Roles](combat/ROLES.md), [Counters](combat/COUNTERS.md), [Genetic Abilities](combat/GENETIC_ABILITIES.md) |
...
- [DECISIONS](DECISIONS.md) — решения baseline и COMBAT/DIV из CANON UPDATE 001–010.
...
- Update 010 конкретизирует COMBAT-028–COMBAT-030: равенства по давности/каталогу, критерии целей и последнее позиционное равенство, независимое ожидание и несколько мягко удерживаемых действий. Это блок №1 из 12 на приёмке, не завершение всего 2D.
- После независимой приёмки и переноса Update 010 в main следующая тема — блок №2 целиком: раунд, Charge, реакции и условия победы, включая факт применения/прерывания и порядок событий. Уже согласованный счётчик повторно не обсуждается без реального дефекта и явной причины. Полные фазы боя, генетический каталог и численная проверка остаются отдельными зависимостями. Численные формулы 2C и связь с генетическим каталогом остаются открытыми зависимостями; Precision решается с таргетингом. В этой задаче следующие этапы не начинаются.
...
**Сейчас: 0/12 завершено, 12 осталось; блок №1 на приёмке.** Фиксация Update 010 выполнена в рабочей ветке; независимая приёмка и перенос в main ещё не выполнены. После успешной приёмки и переноса: **1/12 завершён, 11 осталось** на уровне решений блока. Это не доля готового продукта, не число commits/updates и не число общих OPEN-вопросов.

Основание и полный согласованный список: [Update 010, §8](updates/GENOME_CANON_UPDATE_010.md#8-счётчик-оставшихся-крупных-блоков). Ниже сохранено состояние таблицы при подготовке update; текущий результат фиксации отмечен выше.
...
Счётчик не уменьшается за новый commit. Повторное открытие принятого правила требует реального дефекта и явно указанной причины. Следующая тема после приёмки — блок №2 целиком. Блок №12 не означает запуска итогового ревью в этой задаче.
