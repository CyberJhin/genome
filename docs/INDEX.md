...
Статус и версия разделены: используются только APPROVED, DRAFT, OPEN, NOT_STARTED. APPROVED v0.1 не означает, что пробелы исходников решены. Подразделы наследуют только уже заданный scope одобрения; новые согласованные решения зафиксированы отдельно в CANON UPDATE 001–008; APPROVED не распространяется на неописанную детализацию.
...
| Combat | DRAFT | COMBAT-001–COMBAT-024, только согласованные границы | [Core](combat/COMBAT_CORE.md), [Stats](combat/COMBAT_STATS.md), [Roles](combat/ROLES.md), [Counters](combat/COUNTERS.md), [Genetic Abilities](combat/GENETIC_ABILITIES.md) |
...
| Genetic Abilities | DRAFT | COMBAT-019–COMBAT-024: только качественная модель APPROVED; каталог OPEN | [Genetic Abilities](combat/GENETIC_ABILITIES.md) |
...
- [DECISIONS](DECISIONS.md) — решения baseline и COMBAT/DIV из CANON UPDATE 001–008.
...
- После проверки фиксации и переноса в main следующий узкий вопрос — что считать собственной возможностью применения, когда начинается/сбрасывается ожидание и какое запасное поведение допустимо. Конкретное разрешение равных приоритетов остаётся открытой зависимостью. Численные формулы 2C и связь с генетическим каталогом остаются открытыми зависимостями; Precision решается с таргетингом. В этой задаче следующие этапы не начинаются.
...
Вводная GDS сохранена в Game Vision; вводная DB — в Terminology. BALANCE_MODEL содержит ссылки на существующие определения. COMBAT_CORE содержит основу Update 001 и ссылки на Update 002–008; COMBAT_STATS сохраняет определения Update 002 и качественные правила Update 003; COUNTERS фиксирует взаимодействия Update 004–006. Roles остаётся stub; GENETIC_ABILITIES фиксирует COMBAT-019–COMBAT-024 из Update 007–008 и имеет статус DRAFT. Новый GENETIC_DIVERSITY_MODEL содержит DIV-001–DIV-005; это дополнение к карте исходного baseline, а не перенос или изменение исходных разделов.
